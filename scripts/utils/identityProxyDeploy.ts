...
function sstoreCode(slotNumber, keyType, key, valueType, valueBuf) {
  const buf = ethers.utils.defaultAbiCoder.encode(
    [keyType, valueType],
    [key, slotNumber]
  );
  const slot = ethers.utils.keccak256(buf);
  return Buffer.concat([
    evmPush(
      typeof valueBuf === "string"
        ? Buffer.from(valueBuf.slice(2), "hex")
        : valueBuf
    ),
    evmPush(ethers.utils.arrayify(slot)),
    Buffer.from("55", "hex"),
  ]);
}

function getProxyDeployBytecode(
  masterContractAddr,
  privLevels,
  opts = { privSlot: 0 }
) {
  ...
  const masterAddrBuf = Buffer.from(
    masterContractAddr.slice(2).replace(/^(00)+/, ""),
    "hex"
  );
  if (new Number(masterAddrBuf) > 20) throw new Error("invalid address");
...
export { evmPush, sstoreCode, getProxyDeployBytecode };
